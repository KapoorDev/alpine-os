...
(pkg install wget pv proot tar pulseaudio neofetch -y) > /dev/null
...
if [ "$check" -eq "1" ] > /dev/null 2>&1; then
	...
	*)
	...
	url=https://github.com/KapoorDev/KDev/blob/master/${distro}-rootfs/${archurl}/${distro}-minirootfs-3.10.3-${archurl}.tar.gz?raw=true
...
if [ -d $folder/var ];then
	...
	echo "Check network connectivity and contact devs on Telegram t.me/hacknhaul if problems persist"
